...
Cross-workspace, owner-only view of growth / engagement / platform health / live presence. Gating scaffold shipped (`/admin` route, `User.isPlatformOwner` flag, middleware + layout + route-handler defense in depth). Phase 0 dual-auth gate shipped (`requirePlatformOwner` now accepts either Bearer header or Supabase session cookie). Phase 1 shipped: `/admin` renders growth/engagement/health cards (Arabic RTL) backed by `/api/admin/metrics/{growth,engagement,health}`, each gated and audit-logged. Phase 2 shipped 2026-04-25: live presence card (1-min/5-min active users, per-workspace breakdown gated by membership ≥5 for k-anonymity, 7×24 heatmap, peak-concurrency record) backed by `/api/admin/metrics/presence`; introduced `User.lastActive*` columns + `PlatformStat` singleton + dual middleware/handler activity tracking. Metrics, queries, and UI are designed in a dedicated PRD: **`docs/prd-admin-dashboard.md`**. Phase 3 (time series / sparklines) is next.
