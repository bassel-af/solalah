...
This repository ("solalah") is a **family collaboration platform** evolving from a read-only genealogy viewer. Built with Next.js 15 (App Router) + React 19 + TypeScript, backed by PostgreSQL (Prisma ORM) and Supabase Auth (self-hosted via Docker Compose). The app is RTL (right-to-left) with Arabic as the primary language. See `docs/prd.md` for product requirements, `docs/implementation.md` for how each subsystem currently works, and `docs/auth-provider-decisions.md` for auth architecture decisions.

**Current state**: Phases 1–9, Phase 10b, and Phase 15a are complete; magic link sign-in from Phase 11 is complete; Phase 10c (Tang-bound unlock) and Phase 15b/15c (cascade + pointer undo) are future work. Phase 1 (Auth & Workspace Foundation): email/password + Google OAuth, workspace CRUD, membership/invitations, workspace list UI, policy page. Phase 2 (Editable Family Tree): database-backed tree with full CRUD. Phase 3 (Family-Aware Relationship Editing): add child/spouse/parent, move child between families, marriage events (MARC/MARR/DIV), Hijri date support, calendar preference. Phase 4 (In-Law Visibility & Multi-Root View): re-root on spouse's ancestor, view mode toggle (single/multi), multi-root side-by-side layout, inline spouse family graft expansion. Phase 5 (Branch Pointers): cross-workspace branch linking with live sync, deep copy, stitching rules, pointer management in canvas sidebar. Phase 6a (Islamic Tags): `_UMM_WALAD` flag on families, rada'a (milk kinship) with `RadaFamily`/`RadaFamilyChild` models, 6 API endpoints, sidebar integration, `IndividualPicker` component, workspace feature toggles, kunya (الكنية) field on Individual with `_KUNYA` GEDCOM tag and `enableKunya` workspace toggle. Phase 6b (Export): GEDCOM export (5.5.1 + 7.0) with all Islamic extensions, export dropdown in CanvasToolbar, `@#DHIJRI@` calendar escape, GIVN/SURN sub-tags, GEDCOM injection sanitization. Phase 6c (Import): GEDCOM import for empty workspace trees, parser extended for `_UMM_WALAD`/`_RADA_*` tags, seed helper extended for rada'a, import button in EmptyTreeState. Phase 7 (Advanced Tree Editing): move subtree (replaces single-child move with full branch move, `MoveSubtreeModal`, cycle detection), link existing person as spouse (`IndividualPicker` with exclude set), unlink spouse (delete family or clear spouse slot), security fixes (pointed/synthetic guards, self-marriage/duplicate prevention). Phase 8 (Cascade Delete Warning): `computeDeleteImpact()` reachability analysis with married-in spouse detection, `GET /delete-impact` preview endpoint, enhanced DELETE with cascade mode + stale data protection (409) + server-side name confirmation, `CascadeDeleteModal` component, `executeCascadeDelete()` atomic transaction (break pointers, revoke tokens, clean empty families, audit log), dedicated rate limiter, 31 unit tests. Phase 9 (Audit Log): snapshot-based tree edit history with `enableAuditLog`/`enableVersionControl` workspace toggles, `snapshotBefore`/`snapshotAfter`/`description` on `TreeEditLog`, all 19 mutation routes capture snapshots, `GET /api/workspaces/[id]/tree/audit-log` (admin-only, paginated, filtered, rate-limited), audit log page with diff viewer, CanvasToolbar link, sidebar per-person history, workspace settings toggles. Phase 10a (Layer 1 disk encryption): LUKS2-encrypted Hetzner volume at `/mnt/encrypted` with keyfile-based auto-unlock (see `docs/deployment/layer-1-encryption.md`). Phase 10b (Layer 2 per-workspace application encryption): per-workspace AES-256-GCM data keys wrapped with a master key loaded from `WORKSPACE_MASTER_KEY` env var, validated at module load in `src/lib/db.ts` (fail-fast startup). Crypto primitives in `src/lib/crypto/workspace-encryption.ts` (AES-256-GCM only, 12-byte random nonces, packed `iv||authTag||ciphertext` format, decrypt failures always throw). Domain adapter in `src/lib/tree/encryption.ts` encrypts 15 `Individual` fields, 15 `Family` event fields, and `RadaFamily.notes` as `Bytes?` columns; wraps `TreeEditLog.snapshotBefore`/`snapshotAfter` in `{ _encrypted, data }` envelopes with legacy pass-through. All 9 PII-bearing mutation routes encrypt on write; `dbTreeToGedcomData(tree, workspaceKey)` decrypts on read; branch pointer deep copy re-encrypts with the target workspace's key. Migration script `scripts/encrypt-existing-data.ts` (idempotent, `pnpm encrypt:existing`) converts existing plaintext to ciphertext; companion `scripts/verify-encryption.ts` validates end-to-end. Place names stay plaintext (low privacy value). A post-merge follow-up closed the audit log PII leaks: `TreeEditLog.description` and `payload` are now `Bytes?` columns encrypted via dedicated `encryptAuditDescription`/`encryptAuditPayload`/`decryptAuditDescription`/`decryptAuditPayload` helpers in `src/lib/tree/audit.ts` (payload is encrypted as a whole JSON blob — indexable columns `action`/`entityType`/`entityId`/`userId`/`timestamp` stay plaintext at the top level for queries). The cascade-delete `targetName` leak is now closed inside the encrypted payload envelope. Migration `20260409101126_phase_10b_followup_encrypt_audit_description_payload` and the extended migration script handle the legacy rows. All `TreeEditLog` columns are now encrypted. Operational safeguards: `AdminAccessLog` Prisma model + `logAdminAccess()` helper (`src/lib/audit/admin-access.ts`) scaffolded for future admin-only route wiring; audit-immortal across user deletion, field-length caps, PII-free error logging, 18 unit tests. Migration script `description` fallback hardened with `isLikelyPlaintextUtf8()` guard (`scripts/lib/utf8-guard.ts`). `share-tokens/[tokenId]/route.ts` swallowed audit errors replaced with `logSwallowedAuditError()` (`src/lib/api/swallowed-error-log.ts`). Operator runbook at `docs/encryption.md` covers key generation, backup, recovery, rotation, incident response. 144+ Phase 10b tests across the suite. Phase 15a (Session Undo — Ctrl+Z / Ctrl+Shift+Z): client-side `UndoStackProvider` mounted inside `WorkspaceTreeProvider` (auto-clears on workspace switch via remount keyed on `workspaceId`), per-tab in-memory stack (size 100, FIFO eviction, redo cleared on new push), pure reducer in `src/lib/undo/stack.ts`. Global keyboard hook `useKeyboardUndoRedo` listens on `document`, drops `event.repeat` / `event.isComposing` / `event.defaultPrevented` / focus-in-INPUT-TEXTAREA-contenteditable. Inverse builders in `src/lib/tree/undo-builders.ts` (12 builders covering individual create/update/delete, family create/update/delete, family children add/remove/move, rada'a family create/update/delete). Labels via `buildUndoLabel()` in `src/lib/tree/undo-label.ts` (gendered variants, 40-char truncation). `usePersonActions` wires 14 handler variants via `useUndoableAction` — pushes only after action + `refreshTree` succeed. `UndoStackProvider.undo`/`redo` await `refreshTree` after the inverse so the canvas reverts without page reload; refresh failure drops both stacks and surfaces `ConflictDialog`. Any non-2xx on inverse → conflict dialog + drop stacks; 401/403 → auth-specific dialog variant. Server-side description-prefix: `apiFetch({ isUndo: true })` sends `X-Solalah-Undo: true` → `isUndoRequest()` read in 11 mutation routes → `buildAuditDescription(..., { isUndo })` prefixes audit descriptions with "تراجع عن: " (no new action enum values, no schema changes — deferred to 15b alongside `restoreSnapshot` and `undoOfLogId`). UI: `UndoRedoButtons` in `CanvasToolbar` (icon-only, platform-aware tooltips with Cmd on Mac / Ctrl elsewhere, disabled states, in-flight spinner), `ConflictDialog` built on existing `Modal` (info-blue, no ESC/overlay close), `Toast` carries `role="status" aria-live="polite" aria-atomic="true"`. Cascade delete, branch pointer ops, deep copy, and GEDCOM import are intentionally NOT Ctrl+Z-undoable (scoped to 15b/15c/never). 139+ Phase 15a tests across the suite; total 1977/1977 passing. The tree visualization reads from the database via `GET /api/workspaces/[id]/tree`; static GEDCOM files in `/public/` are preserved for seeding.
