...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  // Phase 10b: encrypt sensitive event fields before the write. `eventFields`
  // also contains plaintext `isUmmWalad` / `isDivorced` booleans which the
  // encrypt helper passes through unchanged.
  const workspaceKey = await getWorkspaceKey(workspaceId);
  const encryptedEventFields = encryptFamilyInput(eventFields, workspaceKey);

  const family = await prisma.family.create({
    // Cast via unknown because Prisma's generated Bytes column type is
    // `Uint8Array<ArrayBuffer>` while Node's `Buffer<ArrayBufferLike>` is a
    // runtime-compatible subclass that TypeScript refuses to widen.
    data: {
      treeId: tree.id,
      husbandId: husbandId ?? null,
      wifeId: wifeId ?? null,
      ...encryptedEventFields,
      children: childrenIds
        ? {
            create: childrenIds.map((individualId) => ({ individualId })),
          }
        : undefined,
    } as unknown as Parameters<typeof prisma.family.create>[0]['data'],
