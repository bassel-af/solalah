          ...
          <>
            ...
            <div className={styles.statsCompact}>
              <div className={styles.statChip}>
                <span className={styles.statChipValue}>{indCount}</span>
                <span className={styles.statChipLabel}>فرد</span>
              </div>
              <div className={styles.statChip}>
                <span className={styles.statChipValue}>{famCount}</span>
                <span className={styles.statChipLabel}>عائلة</span>
              </div>
            </div>
              ...
              {filteredIndividuals.map((person) => (
                <li
                  ...
                  className={clsx(styles.person, {
                    [styles.active]: focusPersonId === person.id,
                    [styles.male]: person.sex === 'M',
                    [styles.female]: person.sex === 'F',
                    [styles.deceased]: person.deceased,
                  })}
            ...
            {/* Advanced Settings Section - Collapsible at Bottom */}
            <div className={styles.settings}>
              <button
                className={clsx(styles.settingsToggle, { [styles.isOpen]: isSettingsOpen })}
                onClick={() => setIsSettingsOpen(!isSettingsOpen)}
                aria-expanded={isSettingsOpen}
              >
                <svg className={styles.settingsIcon} width="16" height="16" viewBox="0 0 24 24" fill="none">
                ...
                <span className={styles.settingsLabel}>خيارات متقدمة</span>
                <svg className={styles.settingsChevron} width="14" height="14" viewBox="0 0 24 24" fill="none">
              ...
              {isSettingsOpen && (
                <div className={styles.settingsContent}>
                  <div className={styles.settingsField}>
                    <label className={styles.settingsFieldLabel}>الجد الأعلى</label>
                    <p className={styles.settingsHint}>اختر الجد الأعلى لتصفية الشجرة</p>
                            ...
                            filteredRoots.map((root) => (
                              <li
                                ...
                                className={clsx(styles.dropdownItem, {
                                  [styles.selected]: root.id === selectedRootId,
                                })}
